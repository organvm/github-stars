...
import type { Manifest, Repository, NormalizationResult } from './types.js';
import { filterValidCategories, validateFramework, canonicalize } from './taxonomy.js';
...
export function normalizeRepository(
  ...
  if (validCategories.length === 0) {
    ...
    const categoriesChanged = originalCategories.length !== validCategories.length ||
      !originalCategories.every((cat, i) => canonicalize(cat) === validCategories[i]);
...
export function normalizeManifest(manifest: Manifest): NormalizationResult {
  ...
  for (const repo of manifest.repositories) {
    const { repo: normalized, changes } = normalizeRepository(repo, manifest.taxonomy);
