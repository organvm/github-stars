...
export function validateManifest(manifest: Manifest): ValidationResult {
  ...
  if (!manifest.taxonomy || !Array.isArray(manifest.taxonomy.categories_allowed)) {
    errors.push({
      repo: '<manifest>',
...
export function formatValidationErrors(result: ValidationResult): string {
  ...
  if (result.errors.length > 0) {
    ...
    for (const error of result.errors) {
      lines.push(`  ❌ ${error.repo}: ${error.message} (${error.field}="${error.value}")`);
    }
  }
  ...
  if (result.warnings.length > 0) {
    ...
    for (const warning of result.warnings) {
      lines.push(`  ⚠️  ${warning.repo}: ${warning.message} (${warning.field}="${warning.value}")`);
    }
  }
